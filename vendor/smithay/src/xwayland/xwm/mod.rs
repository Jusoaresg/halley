...
#[derive(Debug)]
pub struct X11Wm {
    property_jobs: std::sync::mpsc::Sender<X11Surface>,
    initializing_window: bool,
    deferred_events: std::collections::VecDeque<Event>,
    id: XwmId,
...
impl X11Wm {
        ...
        D: 'static,
    {
        ...
        drop(_guard);
        let (property_jobs, jobs) = std::sync::mpsc::channel::<X11Surface>();
        let (completed, completions) = calloop::channel::channel();
        std::thread::Builder::new().name("xwm-properties".into()).spawn(move || {
            while let Ok(surface) = jobs.recv() {
                let result = surface.update_properties().map_err(|err| err.to_string());
                if completed.send((surface, result)).is_err() { break; }
            }
        })?;
        let wm = Self {
            property_jobs,
            initializing_window: false,
        ...
        let completion_dh = dh.clone();
        let completion_token = handle.insert_source(completions, move |event, _, data: &mut D| {
            if let calloop::channel::Event::Msg((surface, result)) = event {
                let is_override_redirect = surface.is_override_redirect();
                let xwm = data.xwm_state(id);
                xwm.initializing_window = false;
                match result {
                    Ok(()) => {
                        xwm.windows.push(surface.clone());
                        if is_override_redirect { data.new_override_redirect_window(id, surface); }
                        else { data.new_window(id, surface); }
                    }
                    Err(err) => warn!(?err, "Failed to initialize X11 window properties"),
                }
                loop {
                    let xwm = data.xwm_state(id);
                    if xwm.initializing_window { break; }
                    let Some(event) = xwm.deferred_events.pop_front() else { break; };
                    if let Err(err) = handle_event(&completion_handle, &completion_dh, data, id, event) {
                        warn!(?err, "Failed to handle deferred X11 event");
                    }
                }
            }
        })?;
        ...
        handle.insert_source(source, move |event, _, data| match event {
            calloop::channel::Event::Msg(event) => {
                if let Err(err) = dispatch_event(&event_handle, &dh, data, id, event) {
                    warn!(id = id.0, err = ?err, "Failed to handle X11 event");
                }
            }
            calloop::channel::Event::Closed => {
                event_handle.remove(completion_token);
    ...
    match event {
        Event::CreateNotify(n) => {
            ...
            let geometry = Rectangle::<i32, Client>::new(
                (i32::from(n.x), i32::from(n.y)).into(),
                (i32::from(n.width), i32::from(n.height)).into(),
            ).to_f64().to_logical(xwm.client_scale.load(Ordering::Acquire)).to_i32_round();
            ...
            // The surface is unpublished while the worker reads its properties.
            // Subsequent X11 events are replayed in order after completion, while
            // the compositor remains free to process input and draw frames.
            xwm.property_jobs.send(surface)?;
            xwm.initializing_window = true;
        }
        Event::MapRequest(r) => {
